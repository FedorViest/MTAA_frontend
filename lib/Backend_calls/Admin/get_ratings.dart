...
class getRatings with ChangeNotifier{
  ...
  getInfo() async {
    try {
      ...
      Response response = await dio.get('http://10.0.2.2:8000/admin/getRatings');
      ...
      for (var item in response.data){
        var order = Rating(item["customer_email"], item["employee_email"], item["Ratings"]["rating"], item["Ratings"]["comment"]);
        ratings.add(order);
